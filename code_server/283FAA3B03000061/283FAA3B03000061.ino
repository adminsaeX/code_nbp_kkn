...
//=========================Define device ===================//
String deviceID = "00000001" ;// 8 digit
String projectID = "NBP1" ;// 4 digit
String api_key = "nlaOIOtgsu";
...
unsigned long interval_power = 100; // time cal power
...
int DCAmp2 = 0;
int DCWatt = 30000; // PV Power
unsigned long DCWh; // PV Energy
unsigned long DCkWh ;// PV Energy kWh
int ACVolt = 0; // AC Volt
int ACAmp = 0; // AC Current
int ACWatt = 0; // AC Power
unsigned long ACWh; // AC Energy
float FlowRate = 0; // Flowrate
unsigned long QTotal; //Water total
...
void setup() {
  ...
  QTotal = EEPROMReadlong(water_total);

  DCWh = EEPROMReadlong(energy_total);
  TempBoard.begin();
  master.begin(9600);
...
{ 
  ...
  if(currentCal - preCal >=interval_calFlow)
  {
    ...
    FlowRate = ((interval_calFlow/(millis()-preCal))*flow_count); // Hz
    FlowRate = FlowRate*FlowScal; //L/s
    FlowRate = FlowRate/3600.0; // L/hr
   // FlowRateM = FlowRate/1000.0 ; // m^3/hr
    QTotal += FlowRateM; //total m^3

    preCal = currentCal ;
    
    attachInterrupt(FlowPin1,pulseCount,FALLING);
    
    sumFlow++;

    if(sumFlow>20)
    {
      EEPROMWritelong(water_total,QTotal);
      sumFlow = 0;
    }
 ...
 void calPower()
 {
   ...
   {
    DCWh += (DCWatt*interval_power)/3600000 ; //Wh
    ...
    if(sumEnergy >=100)
    {
      EEPROMWritelong(energy_total,DCWh);
      sumEnergy=0;
    }
